...
void cdc_init(CDC_config_t * ctx) {
  ...
  UART_RX_DMA->MADDR = uart_rx_buffer;
...
void uart_process_rx(CDC_config_t * ctx) {
  ...
  uint32_t packlen = 0;
  volatile uint32_t current_rx_dma_cnt;
  int ret;

  NVIC_DisableIRQ(USB_IRQn);
  current_rx_dma_cnt = UART_RX_DMA->CNTR;
  if (uart_debug) printf("UART_RX_DMA->CNTR = %ld\n", UART_RX_DMA->CNTR);
  ...
  if (ctx->rx_remain) {
    if (ctx->rxing == 0) {
      ...
      if (packlen) {
        ...
        if (ret == 0) {
          // printf("rxing, packlen = %d, rx_pos = %d\n", packlen, ctx->rx_pos);
          ctx->rxing  = packlen;
          ctx->rx_remain -= packlen;
          ctx->rx_pos += packlen;
          if (ctx->rx_pos >= UART_RX_BUF_SIZE) ctx->rx_pos = 0;
          // Indicate wea are done sending for now, so host can pass data further
...
void uart_process_tx(CDC_config_t * ctx) {
  if (ctx->txing) {
    ...
    if (USART2->STATR & USART_FLAG_TC) {
      ...
      if ((ctx->tx_stop) && (!ctx->tx_remain)) {
        ...
        // USBFS->UEP2_RX_CTRL |= USBFS_UEP_R_RES_ACK;
        ctx->tx_stop = 0;
      }
  ...
  if (ctx->txing == 0 && ctx->tx_remain) {
      ...
      // ctx->tx_pos += to_send; // Increment position
      // if( ctx->tx_pos & 0x3 ) ctx->tx_pos = (ctx->tx_pos + 4) & ~0x3;
      // ctx->tx_remain -= to_send;
