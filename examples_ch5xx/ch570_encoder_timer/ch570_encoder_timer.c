...
int main()
{
	...
	R32_PA_CLR |= (1<<(ENC_BUT & ~PB)); // Falling edge mode for GPIO interrupt
#endif
	...
	R32_ENC_REG_CEND = 0xFF; // Encoder counter highest number, max is 0xFFFFFF

	R8_ENC_INTER_EN = RB_IE_DIR_DEC | RB_IE_DIR_INC; // Enabling encoder increase/decrease interrupt
	NVIC_EnableIRQ(ENCODER_IRQn);

	// Button setup
	R16_PA_INT_MODE |= (1<<(ENC_BUT & ~PB)); // Set GPIO external interrupt to edge mode
	R16_PA_INT_EN |= (1<<(ENC_BUT & ~PB)); // Enable GPIO external interrupt for button pin
	...
	while(1)
	{
		if (enc_val) 
		{
			printf("%d %s\n", (int8_t)R32_ENC_REG_CCNT, enc_val>0?"INC":"DEC");
			enc_val = 0;
		}
		if (button_val)
		{
