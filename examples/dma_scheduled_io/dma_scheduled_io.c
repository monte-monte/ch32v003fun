...
void RunScheduledDMA( const uint32_t * values, const volatile uint32_t ** addresses, const uint16_t * times, int count )
{
	...
	DMA1_Channel7->CFGR = DMA_DIR_PeripheralDST | DMA_CFGR1_PL | DMA_CFGR1_MINC | DMA_CFGR1_TCIE | DMA_CFGR1_EN | DMA_CFGR1_MSIZE_1 | DMA_CFGR1_PSIZE_1 | DMA_CFGR1_CIRC;
...
void RunScheduledDMAFullControl( const volatile uint32_t ** fromaddresses, const volatile uint32_t ** addresses, const uint16_t * times, int count )
{
	...
	DMA1_Channel1->CFGR = DMA_DIR_PeripheralDST | DMA_CFGR1_PL | DMA_CFGR1_MINC | DMA_CFGR1_EN | DMA_CFGR1_MSIZE_1 | DMA_CFGR1_PSIZE_1 | DMA_CFGR1_CIRC;
	...
	TIM2->CH1CVR = 1;
	TIM2->CH2CVR = 1;
	TIM2->CH3CVR = 1;		// Need to get loaded in on the first go.
	TIM2->ATRLR = 16;       // Auto Reload - sets period  (This is how fast each pixel works per set)
	TIM2->CNT = 0;
	TIM2->CTLR1 = TIM2_CTLR1_CEN; // Enable
...
int main()
{
	...
	const uint32_t values   [] = { 3, 2, 1, 0, 3, 2, 2, 3, 0, 1, 2, 3 };

	// Other values
	const uint32_t valuesb  [] = { 1, 2, 1, 2, 1, 2, 0, 1, 2, 3, 2, 3 };

	const volatile uint32_t * fromaddresses[] = {
		&valuesb[0], &valuesb[1], &valuesb[2], &valuesb[3],
		&valuesb[4], &valuesb[5], &valuesb[6], &valuesb[7],
		&valuesb[8], &valuesb[9], &valuesb[10], &valuesb[11] };

	const volatile uint32_t * addresses[] = { 
		&GPIOC->OUTDR, &GPIOC->OUTDR, &GPIOC->OUTDR, &GPIOC->OUTDR,
		&GPIOC->OUTDR, &GPIOC->OUTDR, &GPIOC->OUTDR, &GPIOC->OUTDR,
		&GPIOC->OUTDR, &GPIOC->OUTDR, &GPIOC->OUTDR, &GPIOC->OUTDR,
	};
	// Minimum value 11, it gets stable around 16.
